...
#Mean adn median of the edx set.
mean(edx$rating)
median(edx$rating)
...
#As the figure below shows, we suspect genres effect ratings.
...
#We are minimizing:
# Sum ( (yu,i - mu - bi - bu - bg)^2 + lambda(sum(bi^2) + sum(bu^2) + sum(bg^2)) )
...
finalmodel_validation_rmse <- bind_rows(rmse_results, data.frame(method = "Regularized Movie ＋ User + Genre model on Validation set", RMSE = finalmodel_validation_rmse))
finalmodel_validation_rmse
